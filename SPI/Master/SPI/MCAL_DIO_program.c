...
uint8_t DIO_u8GetPinValue(uint8_t Copy_u8PinN,uint8_t *Copy_u8Value)
{
	uint8_t Local_u8Error=NO_ERROR;        /*Local Error State : NO ERROR*/
		...
		else                           /*Pointer is Pointing to Value and PIN NO entered is not more than 7*/
		{
			...
			if(Copy_u8PinN>=0 && Copy_u8PinN<PIN_PER_PORT)
			{		/*InCase PORTA*/
				(*Copy_u8Value)=READBIT(READ_PORT0,Copy_u8PinN);  /*ASSIGN PIN NO to entered Direction*/
			}
			else if(Copy_u8PinN>=PIN_PER_PORT && Copy_u8PinN<(DIO_PORT_1+1)*PIN_PER_PORT)
			{
												/*InCase PORTB*/
				(*Copy_u8Value)=READBIT(READ_PORT1,(Copy_u8PinN-(PIN_PER_PORT)));      /*ASSIGN PIN NO to entered Direction*/
			}
			else if(Copy_u8PinN>=2*PIN_PER_PORT && Copy_u8PinN<(DIO_PORT_2+1)*PIN_PER_PORT)
			{
											   /*InCase PORTC*/
				(*Copy_u8Value)=READBIT(READ_PORT2,(Copy_u8PinN-(2*PIN_PER_PORT)));     /*ASSIGN PIN NO to entered Direction*/
			}
			else if(Copy_u8PinN>=3*PIN_PER_PORT && Copy_u8PinN<(DIO_PORT_3+1)*PIN_PER_PORT)
			{
											   /*InCase PORTD*/
				(*Copy_u8Value)=READBIT(READ_PORT3,(Copy_u8PinN-(3*PIN_PER_PORT)));     /*ASSIGN PIN NO to entered Direction*/
			}
			else
			{
				/*Other Case Wrong PORT NO*/
...
uint8_t DIO_u8SetPinInputType(uint8_t Copy_u8PinN,uint8_t Copy_u8InputType)
{
	...
	if(Copy_u8PinN>=0 && Copy_u8PinN<PIN_PER_PORT)
	{		/*InCase PORTA*/
		Copy_LocalValue = READBIT(DIRECTION_PORT0,Copy_u8PinN);  /*ASSIGN PIN NO to read direction*/
	}
	else if(Copy_u8PinN>=PIN_PER_PORT && Copy_u8PinN<(DIO_PORT_1+1)*PIN_PER_PORT)
	{
										/*InCase PORTB*/
		Copy_LocalValue = READBIT(DIRECTION_PORT1,(Copy_u8PinN-(PIN_PER_PORT)));      /*ASSIGN PIN NO to read direction*/
	}
	else if(Copy_u8PinN>=2*PIN_PER_PORT && Copy_u8PinN<(DIO_PORT_2+1)*PIN_PER_PORT)
	{
									   /*InCase PORTC*/
		Copy_LocalValue = READBIT(DIRECTION_PORT2,(Copy_u8PinN-(2*PIN_PER_PORT)));     /*ASSIGN PIN NO to read direction*/
	}
	else if(Copy_u8PinN>=3*PIN_PER_PORT && Copy_u8PinN<(DIO_PORT_3+1)*PIN_PER_PORT)
	{
									   /*InCase PORTD*/
		Copy_LocalValue = READBIT(DIRECTION_PORT3,(Copy_u8PinN-(3*PIN_PER_PORT)));     /*ASSIGN PIN NO to read direction*/
	}
	else
	{
		/*Other Case Wrong PORT NO*/
